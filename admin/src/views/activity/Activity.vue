        ...
        <Page  
          ...
          show-elevator></Page>
			...
			<Modal
				...
				@on-ok="confirmSub"
				@on-cancel="cancelSub">
				<Form :model="signModal"
							:label-position="'left'"
							:label-width='80'>
					<FormItem label='赠送币种：' prop="unit">
						<Select v-model="signModal.unit" 
						:disabled="isDisable">
							<Option v-for=" coin in allCoinArr" :value="coin.unit" :key="coin.unit">{{ coin.unit }}</Option>
						</Select>
					</FormItem>
					<FormItem label="赠送数量：" prop="amount">
						<Input v-model="signModal.amount" 
...
import { queryIfEnd, activityQuery, createSign, getCoinName, signDetail, fixSignDetail, earlyCloseSign, memberSignQuery } from '@/service/getData';
									...
									click: () => {
										...
										.then(res => {
											if(!res.code) {
												...
												if(res.data.status==='UNDERWAY') this.isDisable = false;
